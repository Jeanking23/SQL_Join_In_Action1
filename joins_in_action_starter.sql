...
SELECT * FROM movies LEFT JOIN users on movies.movie_id = favorite_movie_id;
...
SELECT * FROM users RIGHT JOIN movies on users.user_id = movies.movie_id;
...
SELECT * FROM movies INNER JOIN users ON movies.movie_id = users.favorite_movie_id;

-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 4 >>>>>>>>>>>>>>>>>>>>>>>
-- Determine which lead studio's movies are favorited by users the most.
-- EXPECTED RESULT: Disney
SELECT movies.lead_studio, SUM(users.favorite_movie_id) AS total_favorites
FROM movies 
INNER JOIN users
ON movies.movie_id = users.favorite_movie_id
GROUP By movies.lead_studio
...
SELECT 	AVG(movies.rotten_tomatoes) AS avg_score
FROM movies 
INNER JOIN users 
ON movies.movie_id = users.favorite_movie_id;

-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 6 >>>>>>>>>>>>>>>>>>>>>>>
-- filmDetermine which two movies are have duplicate entries in the dataset. (HINT: No join is needed for this problem)
